...
## Features
- 홈페이지: 사용자는 사이트에서 데이터베이스에 있는 모든 영화에 대한 정보를 확인할 수 있습니다.
- 영화 검색 및 정보 제공: 영화 제목으로 영화를 검색하고, 검색 결과로 해당 영화에 대한 제목, 포스터 이미지, 줄거리 등의 정보를 제공합니다.
- 평점 및 리뷰 수정: 목록에 있는 영화의 평점과 리뷰를 수정할 수 있습니다.
- 영화 삭제: 목록에 있는 영화를 삭제하여 사람들에게 추천할 영화를 관리할 수 있습니다.
- 영화 추가: 새 영화를 추가하고 포스터 이미지, 출시 연도, 줄거리 등을 가져옵니다.
- 평점에 따라 영화 순위 매기기: 영화를 평점별로 정렬하고 평점에 따른 영화 순위를 표시합니다.
...
### 요구 사항 2 - 영화 평점 및 리뷰를 수정할 수 있을 것
...
### 요구 사항 4 -add.html을 통해 새 영화를 추가할 수 있을 것
add페이지를 작동시킴으로써 영화를 추가하고 API를 사용하여 포스터 이미지, 출시 연도, 영화 설명을 가져올 수 있도록 해야 합니다.
